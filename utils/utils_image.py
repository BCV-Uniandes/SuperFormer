...
def tensor2uint(img):
    img = img.data.squeeze().float().clamp_(0, 1).cpu().numpy() # No lo estamos multiplicando con 4095
    #return np.uint8((img*4095.0).round())
    return img
...
def calculate_ssim_3d(img1, img2, border=0):
    ...
    img1 = img1[border:h-border, border:w-border, border:d-border]
    img2 = img2[border:h-border, border:w-border, border:d-border]
    return structural_similarity(img1, img2, multichannel=False, sigma=1.5, gaussian_weight=True, use_sample_covariance=False, data_range=1.0)
